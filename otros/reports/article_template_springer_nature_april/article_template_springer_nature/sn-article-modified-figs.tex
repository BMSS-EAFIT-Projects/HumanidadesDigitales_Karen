...
\begin{document}
	...
	\author*[1]{\fnm{Biviana Marcela} \sur{Su{\'a}rez-Sierra}}\email{bmsuarezs@eafit.edu.co}\equalcont{These authors contributed equally to this work.}
	...
	\noindent\textbf{Google Scholar Profiles:}
	
	\noindent Karen Melissa Gomez-Montoya: \url{https://scholar.google.com/citations?user=1zQCgF4AAAAJ&hl}
	
	\noindent Biviana Marcela Suárez-Sierra: \url{https://scholar.google.com/citations?user=gVFlz1gAAAAJ&hl}
	...
	%%---------------------------------------------------------
	\keywords{Digital humanities, Text analysis, Natural language processing, Public opinion, Colombia}
	...
	\begin{figure*}[t]
		\centering
		\includegraphics[width=0.9\textwidth]{diagrama_paper_ciencia.png}
	...
	Around 13,000 opinion columns were collected from three widely circulated Colombian print media outlets. These columns were published between January 2018 and December 2020. At this stage, the only selection criterion was that the column consist entirely of written text and not require video or audio to understand its content. The columns were not selected based on any specific topic. For each opinion column, the following metadata were collected: the name of the newspaper, the author, the date, the title, the full text (body) of the column, and the URL. Each text was verified to ensure uniqueness (i.e., no duplicated columns), and author names were standardized. After this process, a unique identification number was assigned to each opinion column. Table \ref{tab:col_autores_periodico} summarizes the distribution of columns and authors by newspaper.
...
\normalsize
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width = 0.47\textwidth]{accuracy_umbral.png}
	...
	Since embeddings had already been used in earlier stages of the analysis to represent corpus fragments, methodological coherence was maintained by employing the same vector representation model (\texttt{embeddinggemma-300m}) for topic modeling. The model was applied to both the full corpus and the science subcorpus, enabling subsequent comparisons between general thematic configurations and those specifically associated with scientific discourse. The implementation in Python allowed control over various components of the process, including the embeddings model, the maximum number of topics to estimate, and the minimum number of documents required to form a topic. In addition, an outlier reduction procedure was applied to decrease the number of unassigned fragments and enforce a more exhaustive classification.
	...
	Table \ref{tab:estadisticos_generales} presents some descriptive statistics of the corpus, such as the number of documents, number of words, number of words per document, among others. 
	
	Furthermore, Figure \ref{fig:nube_comparativa} shows two word clouds: the first corresponds to the entire corpus and the second to the science corpus. These allow us to visualize the relevant vocabulary and appreciate how it varies in both cases.
	...
	\begin{figure}[t]
		...
		\begin{subfigure}{\columnwidth}
			\centering
			\includegraphics[width=\columnwidth]{nube_general.png}
			\caption{Word cloud of the general corpus.}
			\label{fig:nube_general}
		\end{subfigure}
		\begin{subfigure}{\columnwidth}
			\centering
			\includegraphics[width=\columnwidth]{nube_ciencia.png}
	...
	\begin{figure*}[h!]
		\centering
		\includegraphics[width=0.9\textwidth]{porcentaje_mensual_ciencia_linea.png}
	...
	Initially, this topic modeling exercise was performed on the entire corpus, with the aim of
	characterizing the general thematic structure of the opinion columns before focusing the analysis
	on the science subcorpus. To do this, the model was run by varying the parameter \textit{num\_topics},
	which defines the maximum number of topics that can be estimated. Each parameter setting
	produces a different model, understood here as a specific semantic grouping structure in which
	the fragments of the corpus are distributed across a given set of latent topics.
	
	 For each estimated model, the coherence metric $c_v$ was calculated, which evaluates the
	...
	Tables \ref{tab:verbos_ciencia}, \ref{tab:adjetivos_ciencia}, and \ref{tab:sustantivos_ciencia} compile the most relevant verbs, adjectives, and nouns.
	...
	\begin{table}[h!]
		...
		\begin{tabular}{@{}lrrr@{}}
			\toprule
			\textbf{Verb} & \textbf{Total} & \textbf{Science} & \textbf{Sci/Total} \\
			\midrule
	...
	\begin{table}[h!]
		...
		\begin{tabular}{@{}lrrr@{}}
			\toprule
			\textbf{Adjective} & \textbf{Total} & \textbf{Science} & \textbf{Sci/Total} \\
			\midrule
	...
	\begin{table}[h!]
		...
		\begin{tabular}{@{}lrrr@{}}
			\toprule
			\textbf{Noun} & \textbf{Total} & \textbf{Science} & \textbf{Sci/Total} \\
			\midrule
	...
	\textbf{Supplementary material} The supplementary material includes selected excerpts labeled as references to science, presented for illustrative purposes.
